...
function handleUpdate(type, id, w) {
    ...
    let v = 0;
    if (code === "P" || code === "TT" || code === "JF") { v = (type.includes('DHAM') ? 7 : 1); }
...
/**
 * EXPORTATION CORRIGÉE : On capture aussi les cellules <td> avec les IDs v- et e-
 */
function exporterDonnees() {
    const values = {};
    // On sélectionne les inputs, selects, textareas ET les <td> de calcul
    document.querySelectorAll('input, select, textarea, [id^="v-"], [id^="e-"]').forEach(el => { 
        if(el.id) {
            values[el.id] = (el.tagName === 'TD') ? el.innerText : el.value;
        }
    });
...
function restaurerDonnees(data) {
    for (let id in data.values) {
        ...
        if (el) { 
            if(el.tagName === 'TD') el.innerText = data.values[id];
            else el.value = data.values[id]; 
        }
    }
...
function initSignature(id) {
    const canvas = document.getElementById(id); const ctx = canvas.getContext('2d'); ctx.lineWidth = 2; ctx.lineCap = 'round'; let paint = false;
    const getPos = (e) => { const rect = canvas.getBoundingClientRect(); const cx = e.clientX || (e.touches ? e.touches[0].clientX : 0); const cy = e.clientY || (e.touches ? e.touches[0].clientY : 0); const sx = canvas.width / rect.width; const sy = canvas.height / rect.height; return { x: (cx - rect.left) * sx, y: (cy - rect.top) * sy }; };
    ...
    canvas.addEventListener('touchmove', (e) => { if(!paint) return; const p = getPos(e); ctx.lineTo(p.x, p.y); ctx.stroke(); e.preventDefault(); }, { passive: false });
}
...
function updateRecap() {
    for (let c in allCodes) {
        let count = 0;
        document.querySelectorAll('.code-select').forEach(s => { if(s.value === c) { if(c === "Demi P") count += 0.5; else count += 1; } });
